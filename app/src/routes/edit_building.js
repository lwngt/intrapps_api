...
let LOG_FILE_NAME = "edit_building";
...
router.post('/', (req, res) => {
  ...
  if (validation_common_func.check_secretkey(req.headers['apikey']) === false) {
    let response = api_common_func.response_401_0005();
    api_common_func.response_object(res, 401, response, null, LOG_FILE_NAME);
    return;
  }
  ...
  if (validation_common_func.check_download_enabled(req.body.download_enabled) === false) {
    let response = api_common_func.response_400_0442();
    api_common_func.response_object(res, 400, response, null, LOG_FILE_NAME);
    return;
  }
  ...
  let argument_content = {
    "data": {
      ...
      "download_enabled": req.body.download_enabled
    }
  }
...
//          : argument_content  / build_id, release_note, download_enabled
//          : res               / response object
// response : none
...
async function run_process_all(apikey, secretkey, argument_content, res) {
  try {
    ...
    // get update sql text
    let edit_sql = db_common_func.create_update_building_sql(
      argument_content.data.build_id,
      argument_content.data.release_note,
      argument_content.data.download_enabled);
