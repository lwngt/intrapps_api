...
//          : argument_content  / project id (api argument)
//          : res               / response object
// response : none
...
async function run_process_all(apikey, secretkey, argument_content, res) {
  try {
    ...
    // get project detail
    let project_sql
      = db_common_func.create_select_project_sql(
        vendor_id, user_id, argument_content.data.project_id);
    ...
    // get build history
    let build_history_sql
      = db_common_func.create_select_buildings_for_project_build_history_sql(
        argument_content.data.project_id);
    result = await execute_sql(build_history_sql);
    let build_history = result;

    // create project detail response
    let response_data = util_common_func.create_project_api_content(
      sidebar, project[0], build_history);
