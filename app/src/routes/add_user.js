...
async function run_process_all(apikey, secretkey, argument_content, res) {
  try {
    ...
    if (Number(management) === 0 || Number(management_add) === 0) {
    ...
    if (argument_content.data.user_manage_project_ids === undefined
      ...
      // check project_id
      let check_sql = db_common_func.create_projects_of_user_manage_sql(vendor_id, user_id);
      result = await execute_sql(check_sql);

      let check_count = 0;
      result.forEach(project => {
        argument_content.data.user_manage_project_ids.forEach(project_id => {
          if (Number(project_id) === Number(project.project_id)) {
            check_count++;
            insert_user = true;
          }
        });
    ...
    let insert_user_id = result.insertId;

    if (insert_user) {
      // insert user_manage_projects
