...
let LOG_FILE_NAME = "delete_building";
...
//          : argument_content    / build id to delete
//          : res                 / response object
// response : none
...
async function run_process_all(apikey, secretkey, argument_content, res) {
  try {
    ...
    let delete_sql = db_common_func.create_building_sql(
      vendor_id, argument_content.data.build_id, user_id);
    result = await execute_sql(delete_sql);
    let delete_files = [];

    // icon
    if (result[0].build_icon !== undefined && result[0].build_icon !== null) {
      delete_files.push(api_common_func.ICON_LOCAL_DIRECTORY + "/" + result[0].build_icon);
    }
    ...
    if (result[0].type !== undefined
      ...
      && result[0].type !== 0) {
      // Android file delete
      if (result[0].file_name !== undefined
        && result[0].file_name !== null) {
        delete_files.push(api_common_func.APK_LOCAL_DIRECTORY + "/" + result[0].file_name);
      }
    } else if (result[0].type !== undefined
      && result[0].type !== null
      && result[0].type === 0) {
      // iOS file delete
      if (result[0].plist !== undefined
        && result[0].plist !== null) {
        delete_files.push(api_common_func.PLIST_LOCAL_DIRECTORY + "/" + result[0].plist);
      }
      if (result[0].file_name !== undefined
        && result[0].file_name !== null) {
        delete_files.push(api_common_func.IPA_LOCAL_DIRECTORY + "/" + result[0].file_name);
      }
    }
    // delete file execute
    ...
    let build_id_string = String(argument_content.data.build_id);
    ...
    delete_sql = db_common_func.delete_building_sql(argument_content.data.build_id);
    result = await execute_sql(delete_sql);
    if (result === null) {
      let response = api_common_func.response_400_0004();
      api_common_func.response_object(res, 400, response, null, LOG_FILE_NAME);
