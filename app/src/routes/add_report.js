...
//          : argument_content  / build_id, note
//          : res               / response object
// response : none
...
async function run_process_all(apikey, secretkey, argument_content, res) {
  try {
    ...
    // check the build id
    let select_sql = db_common_func.create_building_sql(
    ...
    let edit_sql = null;
    edit_sql = db_common_func.create_report_insert_sql(
      argument_content.data.build_id, user_id, argument_content.data.note);
