...
// get method to get buildings data
router.get('/', (req, res) => {
  ...
  // process
  run_process_all(apikey, secretkey, res);
});
...
async function run_process_all(apikey, secretkey, res) {
  try {
    ...
    let users_sql = db_common_func.create_users_sql(vendor_id, user_id);
    result = await execute_sql(users_sql);
    let users = result;

    // make json response
    let response_data = util_common_func.create_users_api_content(sidebar, users);
