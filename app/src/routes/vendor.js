...
async function run_process_all(apikey, secretkey, res) {
  try {
    ...
    // get admin user list
    let admin_sql = db_common_func.create_admin_users_sql(vendor_id);
    result = await execute_sql(admin_sql);
    let admin = result;

    // make json response
    let response_data = util_common_func.create_vendor_api_content
      (sidebar, vendor[0], admin);
